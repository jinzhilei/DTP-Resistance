...
bool CSystem::SystemUpdate(double t)
{
    ...
    for(k=1; k<=Ntemp; k++)
    {
        _X1Aver=_X1Aver+X1[k];
        _X2Aver=_X2Aver+X2[k];
        _X3Aver=_X3Aver+X3[k];
	}
	
    _X1Aver=_X1Aver/Ntemp;
    _X2Aver=_X2Aver/Ntemp;
    _X3Aver=_X3Aver/Ntemp;
    ...
    for (i=1; i<=Ntemp; i++)
    {
        if(Rand()<p0 && k<_MaxNumCell)
        {
			...
			(*this)(k)._Krs[0]=(*this)(k).fbeta(_NumCell, X1[k], X2[k], X3[k]);
            (*this)(k)._Krs[1]=(*this)(k).fkappa(X2[k]);
            (*this)(k)._Krs[2]=(*this)(k).fdeathrate(X1[k], X3[k]);
            (*this)(k)._Krs[3]=(*this)(k).ftau(X1[k], X3[k]);
        }
        if (k==_MaxNumCell)
        {
            break;
...
void CSystem::Run()
{
    ...
    for (t=0; t<=_MD.T1; t=t+_MD.dt)
    {
        ...
        if(SystemUpdate(t))
        {
            fprintf(fmdsys,"%.4f %.4f %.4f %d %d %d %d %d %d %d %.4f %.4f %.4f\n",t,_Prolif, _NumCell, _NumPoolCell, _N0, _N1, _N2, _N3, _N4, _N5, _X1Aver, _X2Aver, _X3Aver);
...
void CSystem::OutPutSys(int step)
{
	...
	// Edit to change you output items.
        ...
        fprintf(fp,"%d  %6.3f  %6.3f  %6.3f   %6.3f  %6.3f  %6.3f  %6.3f \n",k, (*this)(k)._X[0], (*this)(k)._X[1], (*this)(k)._X[2],  (*this)(k)._Krs[0], (*this)(k)._Krs[1], (*this)(k)._Krs[2], (*this)(k)._Krs[3]);
	}
