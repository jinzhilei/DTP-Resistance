...
struct IMD{
	...
	char drugpar[ComLength]; // drug schedule file.
	...
	int schedule;            // drug schedule ID
//	double Tstart;           // starting time of treatment
//	double Tend;			 // end time of treatment
};					    // MD parameters;
...
struct DrugSchedule{
    int DS_id;               // schedule ID
    char DS_name[50];       //  schedule name
    int DS_count;           // the total number of drug on and off
    int * DS_times;       
	// drug on time 1, drug  off time 1, drug on time 2, drug  off time 2,...
};
